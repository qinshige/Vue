<template>
  <div>
    ...
    <div class="select">
      <ul class="select-bar">
        <v-touch tag="li" v-for="(item, index) in types" :key="index" :class="{'active': index === majorSelected}" @tap="setType(item.type,index)">
          {{item.name}}
        </v-touch>
      </ul>
      <ul class="select-bar" v-if="mins">
        <li data-type="hot">全部</li>
        <v-touch tag="li" :class="{'active': index === minorSelected}" v-for="(minor, index) in mins" :key="index" >
          {{minor}}
        </v-touch>
      </ul>
    </div>
